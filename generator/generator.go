...
import (
    ...
    "html"
    "io"
    "os" // Added for runtime constant loading
    "path/filepath"
    "reflect"
    "strconv"
    "strings"
...
func Generate(template parser.TemplateFile, w io.Writer, opts ...GenerateOpt) (op GeneratorOutput, err error) {
    g := &generator{
        ...
        txtPath:   strings.TrimSuffix(template.FileName, ".templ") + "_templ.txt", // Added for _templ.txt path
    }
...
type generator struct {
    ...
    variableID  int
    childrenVar string
    txtPath     string // Path to _templ.txt file
    options     GeneratorOptions
}

func (g *generator) generate() (err error) {
    // Write constants to _templ.txt
    if err = g.writeConstantsToTxt(); err != nil {
        return fmt.Errorf("write constants to %s: %w", g.txtPath, err)
    }
    ...
    if err = g.writeConstantLoader(); err != nil { // Added to load constants
        return
    }
...
func (g *generator) writeConstantLoader() error {
    _, err := g.w.Write(fmt.Sprintf(`
var templConstants []string
...
func init() {
    ...
    templConstants = strings.Split(string(data), "\n")
}
`, g.txtPath))
    return err
}

func (g *generator) writeConstantsToTxt() error {
    return os.WriteFile(g.txtPath, []byte(strings.Join(g.w.Literals, "\n")), 0644)
}
...
func (g *generator) writeNode(indentLevel int, current parser.Node, next parser.Node) (err error) {
    ...
    // Write trailing whitespace only if needed for HTML rendering, not for markdown tables.
    needed := isInlineOrText(current) && isInlineOrText(next) && !isMarkdownTableNode(current, next)
...
// isMarkdownTableNode checks if the current or next node is part of a markdown table.
func isMarkdownTableNode(current, next parser.Node) bool {
    if curr, ok := current.(parser.StringExpression); ok {
        value := strings.TrimSpace(curr.Expression.Value)
        return strings.Contains(value, "|") || strings.Contains(value, "-----")
    }
    if curr, ok := current.(parser.Text); ok {
        value := strings.TrimSpace(curr.Value)
        return strings.Contains(value, "|") || strings.Contains(value, "-----")
    }
    if curr, ok := current.(parser.CallTemplateExpression); ok {
        // Assume component calls like @mdTableRow are part of markdown tables.
        return true
    }
    if next != nil {
        if nextCurr, ok := next.(parser.StringExpression); ok {
            value := strings.TrimSpace(nextCurr.Expression.Value)
            return strings.Contains(value, "|") || strings.Contains(value, "-----")
        }
        if nextCurr, ok := next.(parser.Text); ok {
            value := strings.TrimSpace(nextCurr.Value)
            return strings.Contains(value, "|") || strings.Contains(value, "-----")
        }
        if nextCurr, ok := next.(parser.CallTemplateExpression); ok {
            return true
        }
    }
    return false
...
func (g *generator) writeCallTemplateExpression(indentLevel int, n parser.CallTemplateExpression) (err error) {
    ...
    // var vn bytes.Buffer
    if _, err = g.w.WriteIndent(indentLevel, "var "+vn+" bytes.Buffer\n"); err != nil {
        return err
    }
    // templ_7745c5c3_Err = TemplateExpression.Render(ctx, &vn)
    if _, err = g.w.WriteIndent(indentLevel, "templ_7745c5c3_Err = "); err != nil {
        return err
    }
    if r, err = g.w.Write(n.Expression.Value); err != nil {
    ...
    g.sourceMap.Add(n.Expression, r)
    if _, err = g.w.Write(".Render(ctx, &"+vn+")\n"); err != nil {
        return err
    }
    if err = g.writeErrorHandler(indentLevel); err != nil {
        return err
    }
    // Write the trimmed output to avoid extra spaces in markdown tables.
    if _, err = g.w.WriteIndent(indentLevel, "_, templ_7745c5c3_Err = templ_7745c5c3_Buffer.WriteString(strings.TrimSpace("+vn+".String()))\n"); err != nil {
        return err
    }
    if err = g.writeErrorHandler(indentLevel); err != nil {
        return err
...
func (g *generator) writeStringExpression(indentLevel int, e parser.Expression) (err error) {
    ...
    // Trim spaces for markdown table expressions (e.g., "|", "-----") and avoid escaping.
    trimmedVn := vn
    if strings.Contains(e.Value, "|") || strings.Contains(e.Value, "-----") {
        trimmedVn = "strings.TrimSpace(" + vn + ")"
        // Write without escaping for markdown delimiters.
        if _, err = g.w.WriteIndent(indentLevel, "_, templ_7745c5c3_Err = templ_7745c5c3_Buffer.WriteString("+trimmedVn+")\n"); err != nil {
            return err
        }
    } else {
        // Use escaping for non-markdown content.
        if _, err = g.w.WriteIndent(indentLevel, "_, templ_7745c5c3_Err = templ_7745c5c3_Buffer.WriteString(templ.EscapeString("+vn+"))\n"); err != nil {
            return err
        }
    }
